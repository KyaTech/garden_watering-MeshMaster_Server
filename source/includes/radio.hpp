...
public:
    ...
    */
    unsigned long sendRadioPayload(RadioPayload &payload) noexcept(false);
    ...
    */
    unsigned long
    sendRequest(string attribute_requested, string additional_value, uint16_t node) noexcept(false);
    ...
    * @return request_id of payload
    */
    unsigned long sendRequest(string attribute_requested, uint16_t node) noexcept(false);
    ...
    unsigned long sendSimpleResponse(SimpleResponse type, radio_payload_struct &payload,
                                     RF24NetworkHeader &header) noexcept(false);

    // function for sending commands without the struct given instead command and additional_value
    unsigned long
    sendCommand(string command, string additional_value, uint16_t node) noexcept(false);

    // function for sending commands without the struct and additional_value given instead command only
    unsigned long sendCommand(string command, uint16_t node) noexcept(false);

    // function for sending registrations
    unsigned long sendRegistration(ModuleType type, int index, int pin) noexcept(false);
